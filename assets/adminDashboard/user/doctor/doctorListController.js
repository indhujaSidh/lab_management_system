$(document).ready(function () {
    ...
    function getData() {
        ...
        $('#table_doctors_list').DataTable().destroy();
        var tbl = $('#table_doctors_list').DataTable({
            "pageLength": 10,
            "searching": true,
            "scrollY": false,
            "ajax": {
                "url": ajax_get_doctor_list,
                "data": {},
                dataSrc: function (response) {
                    var data = [];
                    for (var i = 0, ien = response['payload']['dataSet'].length; i < ien; i++) {
                        var row = [];
                        row[0] = response['payload']['dataSet'][i].firstName +' ' + response['payload']['dataSet'][i].lastName;
                        row[1] = response['payload']['dataSet'][i].specialization ?? "";
                        row[2] = response['payload']['dataSet'][i].contactNumber;
                        row[3] = response['payload']['dataSet'][i].email;
                        ...
                        row[5] = '';
                        row[5] += '<button type="button" title="Edit" class="edit-doctor edit-icon" value="' + response['payload']['dataSet'][i].id + '"><i class="fa fa-pen edit-icon" aria-hidden="true"></i></button>';
                        row[5] += '&nbsp;&nbsp;';
                        row[5] += '<button type="button" class="delete-doctor trash-icon" value="' + response['payload']['dataSet'][i].id + '" title="Delete"><i class="fa fa-trash delete-icon" aria-hidden="true"></i></button>';
                        row[5] += '';
                        data.push(row);
                    }
                    return data;
    ...
    $(document).on('click', '.edit-doctor', function () {
        var id = $(this).val();
        window.location.href = '/admin/dashboard/doctor/editInfo' + '?id=' + id;
    });
    ...
    function changeActiveStatus(id, isActive) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                section.LoadingOverlay("show");
                $.post(ajax_change_doctor_active_status, {
                    id: id,
                    isActive: isActive,
                }, function (result) {
                    section.LoadingOverlay("hide");
